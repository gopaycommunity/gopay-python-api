...
class TestEET(unittest.TestCase):
    ...
    def create_base_eet_payment(self):
        base_eet_payment = {
            ...
            'items': [
                {'name': 'Pocitac Item1', 'amount': '119990', 'count' : '1', 'vat_rate' : VatRate.RATE_4, 'type' : ItemType.ITEM, 'ean' : '1234567890123', 'product_url' : 'https://www.eshop123.cz/pocitac'},
                {'name': 'Oprava Item2', 'amount': '19960', 'count' : '1', 'vat_rate' : VatRate.RATE_3, 'type' : ItemType.ITEM, 'ean' : '1234567890189', 'product_url' : 'https://www.eshop123.cz/pocitac/oprava'}
            ],
            'callback': {
    ...
    def _create_recurrent_eet_payment(self):
        ...
        if "error_code" not in str(result.json):
            print('EET Recurrence: ' + ("NONE" if "'recurrence'" not in str(result.json) else str(result.json['recurrence'])))
        else:
    ...
    def _next_on_demand_eet(self):
        ...
        eet_on_demand_payment = self.payments.create_recurrence(3049524166, eet_next_payment)
        ...
        if "error_code" not in str(eet_on_demand_payment.json):
            ...
            print('EET Payment instrument: ' + ("NONE" if "'payment_instrument'" not in str(eet_on_demand_payment.json) else str(eet_on_demand_payment.json['payment_instrument'])))
            print('EET Recurrence: ' + ("NONE" if "'recurrence'" not in str(eet_on_demand_payment.json) else str(eet_on_demand_payment.json['recurrence'])))
        else:
