    ...
    def test_create_next_on_demand_payment(self):
        ...
        if "error_code" not in str(on_demand_payment.json):
            ...
            print(
                ...
                + (
                    "NONE"
                    if "'payment_instrument'" not in str(on_demand_payment.json)
                    else str(on_demand_payment.json["payment_instrument"])
                )
            )
            print(
                "Recurrence: "
                + (
                    "NONE"
                    if "'recurrence'" not in str(on_demand_payment.json)
                    else str(on_demand_payment.json["recurrence"])
                )
            )
        else:
