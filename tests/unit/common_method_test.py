...
class TestCommonMethods(unittest.TestCase):
    ...
    def test_payment_instrument_root(self):
        instruments_list = self.payments.get_payment_instruments(
            Utils.GO_ID, Currency.CZECH_CROWNS
        )

        if "error_code" not in str(instruments_list.json):
            print(
                "List of enabled payments instruments: \n"
                + "Groups: "
                + str(instruments_list.json["groups"])
            )
            print(
                "---------------------------------------------------------------------------------------------------"
            )
            print(
                "Enabled payments instruments: "
                + str(instruments_list.json["enabledPaymentInstruments"])
            )
        else:
